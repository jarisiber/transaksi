...
class CommodityController extends Controller
{
    ...
    public function store(StoreCommodityRequest $request)
    {
        ...
        // Get year from date_of_purchase and add to data
        $date = $request->input('date_of_purchase');
        $year = date('Y', strtotime($date));

        Commodity::create($data + ['year_of_purchase' => $year]);
    ...
    public function generatePDF()
    {
        ...
        $jaris = env('NAMA_APLIKASI', 'Barang Milik PT...');
        $pdf = Pdf::loadView('commodities.pdf', compact(['commodities', 'jaris']))->setPaper('a4');
    ...
    public function generatePDFIndividually($id)
    {
        ...
        $pdf = Pdf::loadView('commodities.pdfone', compact(['commodity', 'jaris']))->setPaper('a4');
